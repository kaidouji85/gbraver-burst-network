...
const vpcId = cdk.Fn.importValue(`${service}:VpcId`);
const publicSubnetAvailabilityZone = cdk.Fn.importValue(`${service}:PulicNetAvailabilityZone`);
const publicSubnetId = cdk.Fn.importValue(`${service}:PulicSubnetId`);

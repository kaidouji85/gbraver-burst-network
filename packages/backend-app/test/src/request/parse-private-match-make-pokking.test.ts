...
test("PrivateMatchMakePollingはパースできる", () => {
  expect(parsePrivateMatchMakePolling(privateMatchMakePolling)).toEqual(privateMatchMakePolling);
});

test("余計なプロパティは削除される", () => {
  expect(parsePrivateMatchMakePolling({...privateMatchMakePolling, propA: 12, propB: 12})).toEqual(privateMatchMakePolling);
});

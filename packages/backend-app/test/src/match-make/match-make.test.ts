...
const entry1 = { ...EMPTY_CASUAL_MATCH_ENTRY,
  userID: "user01"
};
const entry2 = { ...EMPTY_CASUAL_MATCH_ENTRY,
  userID: "user02"
};
const entry3 = { ...EMPTY_CASUAL_MATCH_ENTRY,
  userID: "user03"
};
const entry4 = { ...EMPTY_CASUAL_MATCH_ENTRY,
  userID: "user04"
};
const entry5 = { ...EMPTY_CASUAL_MATCH_ENTRY,
  userID: "user05"
};
test("先頭から順番にマッチメイクすることができる", () => {
  const entries = [entry1, entry2, entry3, entry4, entry5];
  const result = matchMake(entries);
  const expected = [[entry1, entry2], [entry3, entry4]];

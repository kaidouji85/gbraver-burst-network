...
export async function battleProgressPolling(event: WebsocketAPIEvent): Promise<WebsocketAPIResponse> {
  ...
  const fetchedCommands = await Promise.all([battleCommands.get(battle.players[0].userID), battleCommands.get(battle.players[1].userID)]);
  ...
  const onGameContinue = () => {
    ...
    return Promise.all([...battle.players.map(v => notifier.notifyToClient(v.connectionId, noticedData)), battles.put({ ...battle,
      flowID,

...
export class BattlePlayer02 implements UseCase {
  ...
  async execute(): Promise<void> {
    ...
    const update02 = await battle.progress({type: 'BATTERY_COMMAND', battery: 5});
    console.log(update02);
    const update03 = await battle.progress({type: 'BATTERY_COMMAND', battery: 5});
    console.log(update03);
    const update04 = await battle.progress({type: 'BATTERY_COMMAND', battery: 0});
    console.log(update04);
    const update05 = await battle.progress({type: 'BATTERY_COMMAND', battery: 0});
